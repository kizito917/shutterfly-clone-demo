...
export const getCanvaAuthorization = async () => {
    return new Promise((resolve, reject) => {
        try {
            const url = new URL(endpoints.AUTHORIZE, import.meta.env.VITE_APP_BACKEND_HOST);
...
export const revoke = async () => {
    const url = new URL(endpoints.REVOKE, import.meta.env.VITE_APP_BACKEND_HOST);
...
export const checkForAccessToken = async () => {
    const url = new URL(endpoints.TOKEN, import.meta.env.VITE_APP_BACKEND_HOST);
    const response = await fetch(url, { credentials: "include" });
